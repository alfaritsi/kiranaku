...
class Datatables
{
	...
	public function __construct()
	{
		...
		/**
		 * Added by ASY
		 * extend buffer size of server
		 * start
		 */
		ini_set("client_buffer_max_kb_size", 102400);
	...
	/**
	 * Generates the GROUP START portion of the query
	 *
	 * @param string $column
	 *
	 * @return mixed
	...
	/**
	 * Generates the GROUP END portion of the query
	 *
	 * @param string $column
	 *
	 * @return mixed
	...
	/**
	 * Builds all the necessary query segments and performs the main query based on results set from chained
	 * statements
	 *
	 * @param string $output
	...
	private function get_filtering()
	{
		...
		/**
		 * Added by ASY
		 * validation if $mColArray is string convert into array
		 * start
		 */
		if (gettype($mColArray) == "string") $mColArray = json_decode($mColArray, true);
		/**
		 *end
		 */
		...
		/**
		 * Added by ASY
		 * validation if $search is string convert into array
		 * start
		 */
		if (gettype($search) == "string") $search = json_decode($search, true);
		/**
		 *end
		 */
			...
			for ($i = 0; $i < count($mColArray); $i++) {
				if ($mColArray[$i]['searchable'] == 'true' && !array_key_exists($mColArray[$i]['data'], $this->add_columns)) {
					if ($this->check_cType()) {
						...
						/**
						 * Added by ASY
						 * if table is join table, use name param as filter
						 * if table is not join table, use data param as filter
						 * start
						 **/
						//$sWhere .= $this->select[$mColArray[$i]['data']]. " LIKE '%" . $sSearch . "%' OR ";
						if (array_key_exists($mColArray[$i]['data'], $this->select) == true) {
							$filter = $this->select[$mColArray[$i]['data']];
							$sWhere .= $filter . " LIKE '%" . $sSearch . "%' OR ";
							$sWhere .= $filter . " LIKE '%" . htmlentities($sSearch) . "%' OR ";
						} elseif (array_key_exists($mColArray[$i]['name'], $this->select) == true) {
							$filter = $this->select[$mColArray[$i]['name']];
							$sWhere .= $filter . " LIKE '%" . $sSearch . "%' OR ";
							$sWhere .= $filter . " LIKE '%" . htmlentities($sSearch) . "%' OR ";
						} else if (strpos($mColArray[$i]['data'], '.') !== false) {
							$sWhere .= $mColArray[$i]['data'] . " LIKE '%" . $sSearch . "%' OR ";
							$sWhere .= $mColArray[$i]['data'] . " LIKE '%" . htmlentities($sSearch) . "%' OR ";
						} else
							$sWhere .= "";
						/**
						 *end
						...
						$sWhere .= $this->select[$this->columns[$i]] . " LIKE '%" . htmlentities($sSearch) . "%' OR ";
					}
				}
			}
		}
	...
	private function produce_output($output, $charset)
	{
		 ...
		 * start
		 */
		$isJson = $this->ci->general->isJson($aaData);
		if ($isJson === FALSE) {
			$row_modif = array();
			foreach ($aaData as $key => $value) {
				foreach (array_keys($value) as $i => $v) {
					// $value[$v] = utf8_encode($value[$v]);
					$encoding_from = mb_detect_encoding($value[$v], ['UTF-8', 'ISO-8859-1'], true);
					$value[$v] = $value[$v] && (is_string($value[$v]) || floatval($value[$v]) == 0) ? $this->ci->general->convertCharset($value[$v], $encoding_from, 'UTF-8') : $value[$v];
				}
				$row_modif[] = $value;
			}
			$aaData = $row_modif;
		...
		if ($output == 'json') {
			...
			/**
			 * Comment by ASY
			 * mssql driver cannot read special char from sql server db, so json_encode cannot be done
			 * start
			 */
			if ($charset == 'utf-8')
	...
	private function exec_replace($custom_val, $row_data)
	{
		...
		if (isset($custom_val['replacement']) && is_array($custom_val['replacement'])) {
			...
			foreach ($custom_val['replacement'] as $key => $val) {
				...
				} else if (in_array($sval, $this->columns))
	...
	private function check_cType()
	{
		 ...
		 * start
		 */
		if (gettype($column) == "string") $column = json_decode($column, true);
		/**
		 * end
		 */
