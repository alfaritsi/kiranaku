...
<?php
	...
	if($jumlah[0]->pengguna=='hrga'){
		...
		$list_kolom = '
				{dataField: "pabrik",caption: "Pabrik",groupIndex: 0,fixed: true, autoExpandGroup: false, width:200},
				{dataField: "nomor",caption: "Nomor"},
				{dataField: "lokasi",caption: "Lokasi"},
				{dataField: "sublokasi",caption: "Sub Lokasi"},
				{dataField: "area",caption: "Area"},
				{dataField: "jenis",caption: "Jenis"},
				{dataField: "merk",caption: "Merk"},
				{dataField: "ratio",caption: "Ratio"},
				{dataField: "nomorsap",caption: "Nomor SAP"},
				{dataField: "jamjalan",caption: "Jam Jalan"},
		...
		// $n 	  = 0;
		$list = "";
		// echo json_encode($asset_det);
		foreach($asset_det as $dt){
			
			// $total_aset 		= $dt->tot_beroperasi+$dt->tot_rusak;
			// $persen_beroperasi 	= ($total_aset!=0)?($dt->tot_beroperasi/$total_aset)*100:0;
			// $persen_rusak 		= ($dt->tot_rusak!=0)?100-$persen_beroperasi:0;
			
			$kondisi 	= $this->generate->kirana_decrypt($dt->id_kondisi) == '1'? 
							"Beroperasi" 	: "Tidak Beroperasi";
            $status 	= $dt->na == 'n' 		? "AKTIF" 		: "NON AKTIF";
            $merk 		= $dt->nama_merk." - ".$dt->nama_merk_tipe;
			$list .= "{
				'ID'				:'".$dt->id_aset."',
				'pabrik'			:'".$dt->nama_pabrik."',
				'nomor'				:'".$dt->nomor."',
				'lokasi'			:'".$dt->nama_lokasi."',
				'sublokasi'			:'".$dt->nama_sub_lokasi."',
				'area'				:'".$dt->nama_area."',
				'jenis'				:'".$dt->nama_jenis."',
				'merk'				:'".$dt->nama_merk."',
				'ratio'				:'".$dt->ratio."',
				'nomorsap'			:'".$dt->nomor_sap."',
				'jamjalan'			:'".$dt->jam_jalan."',
				'kondisi'			:'".$kondisi."',
				'status'			:'".$status."'
			},";
		}
