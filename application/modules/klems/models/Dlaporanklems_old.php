...
class Dlaporanklems extends CI_Model{
	...
	function get_data_tahap_distinct($id_tahap=NULL, $all=NULL){
		$this->general->connectDbPortal();
		$this->db->select('distinct(tbl_tahap.nama)');
	...
	function get_data_peserta($id_peserta=NULL, $all=NULL, $regional_in=NULL, $nik_in=NULL, $posisi_in=NULL, $program_in=NULL, $pabrik_in=NULL, $tahun=NULL, $awal=NULL, $akhir=NULL, $sertifikat=NULL){
		...
		$this->db->select("
						CAST(
						   (SELECT CONVERT(VARCHAR, ISNULL(
							  (select 
								avg(tbl_batch_score.score) 
								from tbl_batch_score 
								left join tbl_batch_nilai on tbl_batch_nilai.id_batch_nilai=tbl_batch_score.id_batch_nilai
								left join tbl_batch tbl_batch4 on tbl_batch4.id_batch=tbl_batch_score.id_batch
								where
								tbl_batch_score.na='n' 
								and tbl_batch4.id_tahap=tbl_tahap.id_tahap
								and tbl_batch_score.id_batch in (select tbl_batch2.id_batch from tbl_batch tbl_batch2 where tbl_batch2.id_program_batch=tbl_program_batch.id_program_batch)
								and tbl_batch_score.id_peserta in (select tbl_peserta2.id_peserta from tbl_peserta tbl_peserta2 where tbl_peserta2.id_karyawan=tbl_karyawan.id_karyawan and tbl_peserta2.id_batch in(select tbl_batch3.id_batch from tbl_batch tbl_batch3 where tbl_batch3.id_program_batch=tbl_program_batch.id_program_batch ))
							  )
						   ,0))+RTRIM(',')
							  FROM tbl_tahap
							 WHERE 
							 tbl_tahap.na='n' and tbl_tahap.del='n'
							 ORDER BY tbl_tahap.id_tahap
							  FOR XML PATH ('')) as VARCHAR(MAX) )as list_nilai_tahap
						  ");
		$this->db->select("(select tbl_karyawan.nama from tbl_karyawan where tbl_karyawan.id_karyawan=tbl_program_batch.ttd_kiri) as nama_ttd_kiri");
		$this->db->select("(select tbl_karyawan.nama from tbl_karyawan where tbl_karyawan.id_karyawan=tbl_program_batch.ttd_kanan) as nama_ttd_kanan");
		...
		// $this->db->select("(select top 1 zdmom0001.posisi from ".DB_DEFAULT.".dbo.zdmom0001) as posisi_batch");		
		
		$this->db->from(DB_PORTAL.'.dbo.tbl_peserta');
		// $this->db->join('tbl_karyawan', 'tbl_karyawan.nik = tbl_peserta.id_karyawan
										 // AND tbl_karyawan.na = \'n\'', 'left');		
		$this->db->join(DB_PORTAL.'.dbo.tbl_karyawan', 'tbl_karyawan.nik = tbl_peserta.id_karyawan
										 ', 'left');		
		$this->db->join(DB_PORTAL.'.dbo.tbl_user', 'tbl_user.id_karyawan = tbl_karyawan.id_karyawan
										 AND tbl_user.na = \'n\'', 'left');		
		$this->db->join(DB_PORTAL.'.dbo.tbl_batch', 'tbl_batch.id_batch = tbl_peserta.id_batch
										 AND tbl_batch.na = \'n\'', 'left');	
		$this->db->join(DB_PORTAL.'.dbo.tbl_program_batch', 'tbl_program_batch.id_program_batch = tbl_batch.id_program_batch 
										 AND tbl_program_batch.na = \'n\'', 'left');		
		$this->db->join(DB_PORTAL.'.dbo.tbl_program', 'tbl_program.id_program = tbl_program_batch.id_program 
										 AND tbl_program.na = \'n\'', 'left');		
		$this->db->join(DB_PORTAL.'.dbo.tbl_tahap', 'tbl_tahap.id_tahap = tbl_batch.id_tahap 
										 AND tbl_program.na = \'n\'', 'left');		
		...
		// if($tahun != NULL){
			// $this->db->where("YEAR(tbl_batch.tanggal_awal)='".$tahun."' or YEAR(tbl_batch.tanggal_awal)= '".$tahun."'");
		// }
		if($sertifikat != NULL){
