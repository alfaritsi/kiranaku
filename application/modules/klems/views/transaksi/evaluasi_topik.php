						...
						if($feedback_pertanyaan){
							foreach($feedback_pertanyaan as $tanya){
								...
								foreach($tanya as $t){
									...
									foreach($feedback_nilai as $fn){
										$nama  = 'nilai_'.$fn->id_feedback_nilai.'_'.$t->id_feedback_pertanyaan;
										if(($t->id_karyawan==base64_decode($this->session->userdata("-nik-")))and($t->id_batch==$batch[0]->id_batch)and($t->id_feedback_nilai==$fn->id_feedback_nilai)){
											echo"
												<td align='center'><input checked='checked' type='radio' name='$nama' onclick=\"save_evaluasi('".$batch[0]->id_batch."','".base64_decode($this->session->userdata("-nik-"))."','".$t->id_feedback_pertanyaan."','".$fn->nilai."')\"></td>
											";
										}else{
											echo"
												<td align='center'><input type='radio' name='$nama' onclick=\"save_evaluasi('".$batch[0]->id_batch."','".base64_decode($this->session->userdata("-nik-"))."','".$t->id_feedback_pertanyaan."','".$fn->nilai."')\"></td>
											";
										}
									}		
									echo "</tr>";
...
<script src="<?php echo base_url() ?>assets/apps/js/klems/transaksi/evaluasi_topik.js"></script>
