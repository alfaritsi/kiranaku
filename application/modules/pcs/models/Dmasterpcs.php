...
class Dmasterpcs extends CI_Model{
	function get_data_jns_formula($id_mjenis=NULL, $all=NULL, $jenis=NULL, $id_mjenis_in=NULL){
		...
		$this->db->order_by('tbl_pcs_mjenis.jns_formula', 'ASC');
		$query 	= $this->db->get();
		$result	= $query->result();
	...
	function get_data_pegrup($id_mpegrup=NULL, $all=NULL, $nama=NULL, $id_mpegrup_in=NULL){
		...
		$this->db->order_by('tbl_pcs_mpegrup.nama_grup', 'ASC');
		$query 	= $this->db->get();
		$result	= $query->result();
	...
	function get_data_lembur($plant=NULL, $all=NULL){
		$this->general->connectDbPortal();

		$this->db->select('tbl_pcs_mlembur.*,
						   CASE
						   		WHEN tbl_pcs_mlembur.active = 1 THEN \'<span class="label label-success">ACTIVE</span>\'
						   		ELSE \'<span class="label label-danger">NOT ACTIVE</span>\'
						   END as label_active');
		$this->db->from('tbl_pcs_mlembur');
		if($plant != NULL){
			$this->db->where('tbl_pcs_mlembur.kode_pabrik', $plant);
		}
		if($all == NULL){
			$this->db->where('tbl_pcs_mlembur.active', 1);
		}
		$this->db->order_by('tbl_pcs_mlembur.kode_pabrik', 'ASC');
		$query 	= $this->db->get();
		$result	= $query->result();
	...
	function get_data_COA($saknr=NULL, $gltxt=NULL, $not_grup_table=NULL){
		$this->general->connectDbDefault();

		$string 	= "SELECT DISTINCT ZDMFICO04.SAKNR as id,
			 				  ZDMFICO04.SAKNR,
		...
		$string 	.= " WHERE 1=1";
		if($saknr !== NULL){
			$string 	.= " AND ZDMFICO04.SAKNR = '".$saknr."'";
		}
		if($gltxt !== NULL){
			$string 	.= " AND ZDMFICO04.GLTXT LIKE '%".$gltxt."%' ESCAPE '!' COLLATE SQL_Latin1_General_CP1_CI_AI";
		}
		if($not_grup_table !== NULL){
			$string 	.= " AND ZDMFICO04.SAKNR NOT IN (SELECT not_grup_table.saknr COLLATE SQL_Latin1_General_CP1_CS_AS
														   FROM [".DB_PORTAL."].dbo.".$not_grup_table." as not_grup_table)";
		}
		$string 	.= " ORDER BY ZDMFICO04.GLTXT ASC";
	...
	function get_data_norma($id_mnorma=NULL, $all=NULL){
		$this->general->connectDbPortal();

		$this->db->distinct(0);
		$this->db->select('tbl_pcs_mnorma.*');
		$this->db->from('tbl_pcs_mnorma');
		if($id_mnorma !== NULL){
			$this->db->where('tbl_pcs_mnorma.id_mnorma', $id_mnorma);
		}
		if($all == NULL){
			$this->db->where('tbl_pcs_mnorma.active', 1);
		}
		$this->db->order_by('tbl_pcs_mnorma.norma', 'ASC');
		$query 	= $this->db->get();
		$result	= $query->result();
