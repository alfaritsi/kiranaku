...
class Dsettingdepo extends CI_Model{
	function get_data_user_autocomplete($cari = NULL) {
		...
		if ($cari !== NULL) {
			...
			$this->db->where(" tbl_karyawan.nik not in (select tbl_depo_user_role.[user] from tbl_depo_user_role where tbl_depo_user_role.na='n')");
		}
	...
	function get_data_user_role($conn = NULL, $id_user_role = NULL, $active = NULL, $deleted = 'n', $user = NULL) {
		...
		$this->db->select('tbl_depo_user_role.*');
		$this->db->select('CASE
								WHEN tbl_depo_user_role.na = \'n\' THEN \'<span class="label label-success">AKTIF</span>\'
								ELSE \'<span class="label label-danger">NON AKTIF</span>\'
						   END as label_active');
		$this->db->select('tbl_depo_role.nama as nama_role');				   
		$this->db->select('tbl_depo_role.tipe_user');				   
		$this->db->select("CASE
								WHEN tbl_depo_role.tipe_user = 'nik' THEN (select tbl_karyawan.nama from tbl_karyawan where CONVERT(varchar(10), tbl_karyawan.nik)=tbl_depo_user_role.[user])
								ELSE 'Jumlah '+ CAST((select count(*) from tbl_karyawan where tbl_karyawan.posst=tbl_depo_user_role.[user] and tbl_karyawan.na='n') AS varchar)
						   END as caption_user");
		...
		if ($id_user_role !== NULL) {
			$this->db->where('tbl_depo_user_role.id_user_role', $id_user_role);
		}
		if ($active !== NULL) {
			$this->db->where('tbl_depo_user_role.na', $active);
		}
		if ($deleted !== NULL) {
			$this->db->where('tbl_depo_user_role.del', $deleted);
		}
		if ($user !== NULL) {
			$this->db->where('tbl_depo_user_role.user', $user);
		}
		$query  = $this->db->get();
		$result = $query->result();
