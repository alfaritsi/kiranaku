...
Class Setting extends MX_Controller{
		...
		if ($action) {
			switch ($param) {
				case 'user':
					...
					$return = $this->general->set($action, "tbl_depo_user_role", array(
						array(
							'kolom' => 'id_user_role',
	...
	private function get_user_role($array = NULL, $id_user_role = NULL, $active = NULL, $deleted = NULL) {
		$user_role	= $this->dsettingdepo->get_data_user_role("open", $id_user_role, $active, $deleted);
	...
	private function save_user($param) {
		...
		if ($id_user_role!=NULL){	
			...
			$this->dgeneral->update("tbl_depo_user_role", $data_row, array(
				array(
					'kolom' => 'id_user_role',
					'value' => $id_user_role
				)
			...
			$ck_user 	= $this->dsettingdepo->get_data_user_role(NULL, NULL, NULL, NULL, $user);
			if (count($ck_user) != 0){ 
			...
			$this->dgeneral->insert("tbl_depo_user_role", $data_row);
		}
    ...
    {
        if (isset($_GET['q'])) {
			$data	= $this->dsettingdepo->get_data_user_autocomplete($_GET['q']);
            $data_json = array(
    ...
    {
        if (isset($_GET['q'])) {
			$data	= $this->dsettingdepo->get_data_posisi_autocomplete($_GET['q']);
            $data_json = array(
                "total_count"        => count($data),
                "incomplete_results" => false,
                "items"              => $data
