	 ...
	 //        }
		...
		function get_data_achv($conn = NULL, $type = NULL, $pabrik = NULL , $tahun = NULL, $bulan = NULL , $minggu = NULL, $nik = NULL  ) {
			...
			if($type == 'triger'){
				...
				$var_pabrik = array('ABL1','DWJ1','DWJ2','KUT1','KMP1','KPT','KPR1','KSP1','KWI1','KJP1','NKP1','NSI1','PSU1','TSS1');
				$hasil = "";
				if($tahun != NULL ){
					$tahun = $tahun;
				} else {
		...
		function get_week($conn = NULL, $date = NULL, $display= NULL, $month=NULL, $year=NULL) {
			...
			if($display !== NULL){
				$this->db->where(" ZKISSTT_0138.WKMNT <= portal_dev.dbo.GetWeekendDaysCount(CONVERT(DATE, CAST(YEAR(ZKISSTT_0138.ACTDT) as varchar(4))+'-'+ CAST(MONTH(ZKISSTT_0138.ACTDT) as varchar(2))+'-01')) ");
