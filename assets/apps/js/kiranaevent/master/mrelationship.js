$(document).ready(function () {
	...
	$(document).on("click", "button[name='action_btn']", function (e) {
		var empty_form = validate('.form-master-relationship',true);
		if (empty_form == 0) {
			var isproses = $("input[name='isproses']").val();
			if (isproses == 0) {
				$("input[name='isproses']").val(1);
				var formData = new FormData($(".form-master-relationship")[0]);
				$.ajax({
					url: baseURL + 'kiranaevent/master/save/relationship',
					type: 'POST',
					dataType: 'JSON',
					data: formData,
					contentType: false,
	...
	$(document).on("click", ".edit", function (e) {
		...
		$.ajax({
			url: baseURL + "kiranaevent/master/get/data_relationship",
			type: 'POST',
			dataType: 'JSON',
			data: {
			...
			success: function (data) {
				if(data) {
					$.each(data, function(i,v){
						// console.log(data);
						$("input[name='id_relationship']").val(v.id_hubungan);						
						$("input[name='relationship_fieldname']").val(v.hubungan);
	...
	$(document).on("click", ".nonactive, .setactive, .delete", function (e) {
		...
		$.ajax({
			url: baseURL + "kiranaevent/master/set/relationship",
			type: 'POST',
			dataType: 'JSON',
			data: {
...
function get_datas(aktif=null){
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            t.clear().draw();
        	$.each(data, function(i,v){
        		var hubungan = v.hubungan+"<br>"+v.label_active+"<br>";

        		//set action button
        		var action = "";
        		if(v.na == 'n' && v.del == 'n'){
                    action = "<li><a href='#' class='edit' data-edit='"+v.id_hubungan+"'>"
                    			+"<i class='fa fa-pencil-square-o'></i> Edit</a></li>"
                          	+"<li><a href='#' class='nonactive' data-nonactive='"+v.id_hubungan+"'>"
                          		+"<i class='fa fa-eye-slash'></i> Non Aktif</a></li>"
                          	+"<li><a href='#' class='delete' data-delete='"+v.id_hubungan+"'>"
                          		+"<i class='fa fa-trash-o'></i> Hapus</a></li>";
                    
                }
                if(v.na == 'y' && v.del == 'n'){
                    action = "<li><a href='#' class='setactive' "
                    			+"data-setactive='"+v.id_hubungan+"'><i class='fa fa-check'></i> Set Aktif</a></li>";
                }
