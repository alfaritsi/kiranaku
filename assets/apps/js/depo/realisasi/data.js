$(document).ready(function() {
    ...
    $(document).on("click", ".history", function() {
        ...
        $.ajax({
			url: baseURL + 'depo/realisasi/get/history',
            type: 'POST',
            dataType: 'JSON',
            data: {
                nomor: nomor
            ...
            success: function(data) {
				var det_penutupan	= "";
					det_penutupan	+= 		'<table class="table table-bordered datatable-vendor">';
					det_penutupan	+= 		'	<thead>';
					det_penutupan	+= 		'		<tr>';
					det_penutupan	+= 		'			<th>Nomor Penutupan</th>';
					det_penutupan	+= 		'			<th>Tanggal Status</th>';
					det_penutupan	+= 		'			<th>Status</th>';
					det_penutupan	+= 		'			<th>Comment</th>';
					det_penutupan	+= 		'		</tr>';
					det_penutupan	+= 		'	</thead>';
					det_penutupan	+= 		'	<tbody>';
                ...
                $.each(data, function(i, v) {
					det_penutupan	+= 		'		<tr>';
					det_penutupan	+= 		'			<td>'+v.nomor_specimen+'</td>';
					det_penutupan	+= 		'			<td>'+v.tanggal_format+'<br>'+v.jam_format+'</td>';
					det_penutupan	+= 		'			<td>'+v.action.toUpperCase()+' OLEH :<br><span class="label label-info">'+v.role_approval+' : '+v.nama_approval+'</span></td>';
					det_penutupan	+= 		'			<td>'+v.label_catatan+'</span></td>';
					det_penutupan	+= 		'		</tr>';
                });
					det_penutupan	+= 		'	</tbody>';
					det_penutupan	+= 		'</table>';
					$("#histori_penutupan").html(det_penutupan);
...
function datatables_ssp() {
    ...
    var mydDatatables = $("#sspTable").DataTable({
		...
		order: [[3, 'desc']],
        ajax: {
			url: baseURL + 'depo/realisasi/get/data/bom',
            type: 'POST',
            data: function(data) {
        ...
        columns: [
            ...
            },
            {
                ...
                "width": "5%",
                "render": function(data, type, row) {
					var url_edit 	= baseURL + "depo/realisasi/edit/" + row.nomor_format;
					var url_detail 	= baseURL + "depo/realisasi/detail/" + row.nomor_format;
					...
					if((row.status_realisasi==row.level)&&(row.status_realisasi==10)){
