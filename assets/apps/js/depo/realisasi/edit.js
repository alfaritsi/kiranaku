...
$(document).ready(function () {
    ...
    $.ajax({
			...
			$.each(data, function (i, v) {
                ...
                if (v.arr_data_sdm) {
					...
					$.each(v.arr_data_sdm, function(a, b){
						...
						output += "	</td>";
						output += "	<td>";
						output += "		<input type='hidden' name='nik[]' value='"+b.nik+"'/>";						
                ...
                if (v.arr_data_asset) {
					...
					$.each(v.arr_data_asset, function(a, b){
						...
						output += "	<td>";
						output += "		<input type='hidden' name='asset[]' value='"+b.kode+"'/>";						
                ...
                if (v.arr_data_lain) {
					...
					$.each(v.arr_data_lain, function(a, b){
						...
						output += "	<td>";
						output += "		<input type='hidden' name='lain_nama[]' value='"+b.nama+"'/>";
        ...
        complete: function () {
			//control input realisasi
			$('#form_realisasi_penutupan_depo_edit .form-control').prop('disabled', true);
			$('#form_realisasi_penutupan_depo_edit .input-control').prop('disabled', false);
	...
	$(document).on("click", "button[name='action_btn']", function(e){
		var empty_form = validate("#form_realisasi_penutupan_depo_edit");
		if(empty_form == 0){
	    	var isproses 		= $("input[name='isproses']").val();
	    	if(isproses == 0){
	    		$("input[name='isproses']").val(1);
		    	var formData = new FormData($("#form_realisasi_penutupan_depo_edit")[0]);

				$.ajax({
					url: baseURL+'depo/realisasi/save/realisasi',
					type: 'POST',
					dataType: 'JSON',
					data: formData,
					contentType: false,
					...
					success: function (response) {
						if (response) {
							...
							if (response.sts == "OK") {
								icon = "success";
								reload = baseURL + "depo/realisasi/approve";
							}
