				...
				if (data) {
					...
					if (data.length > 0) {
						$.each(data, function (i, v) {
							...
							checkImage(baseURL + first_img + '?' + new Date().getTime(), ".product-layout #product" + i + " img", baseURL + default_img + '?' + new Date().getTime());
							...
							output += '			<button type="button" class="btn btn-default ' + lebar_detail_btn + ' detail_item" data-kode="' + v.MATNR + '" data-num="' + v.ITNUM + '" >FOTO <span>'+v.nama+'</span> <i class="fa fa-upload"></i></button>';
							output += '		</div>';
							output += '	</div>';
							output += '</div>';
						});
...
$(document).ready(function () {
    ...
    $(document).on("click", "button[name='add_biaya_sdm']", function () {
        ...
        output += "	</td>";
        output += "	<td>";
        output += "		<input type='text' class='angka form-control text-center' name='nik_sdm[]' value='' placeholder='NIK'  required='required' />";
        output += "	</td>";
        output += "	<td>";
