$(document).ready(function () {
    ...
    $(document).on("click", "form[name='form-transaksi-jadwal-pm'] button[name='action_btn']", function (e) {
        ...
        if (empty_form == 0) {
            ...
            if (isproses == 0) {
                ...
                $.ajax({
                    url: baseURL + 'asset/maintenance/save/fo/bulk_jadwal', //???
                    type: 'POST',
                    dataType: 'JSON',
                    data: formData,
                    contentType: false,
    ...
    $(document).on("change", "#id_jenis", function (e) {
        ...
        if (!KIRANAKU.isNullOrEmpty(id_jenis)) {
            $.ajax({
                url: baseURL + 'asset/transaksi/get/periode/fo',
                type: 'POST',
                dataType: 'JSON',
                data: {
                    id_jenis: id_jenis
                ...
                if (data && data.data.length > 0) {
                    ...
                    $.each(dataAset, function (i, d) {
                        table.DataTable().row.add([
                            ...
                            d.nomor + ' <br> ' + d.nama_kategori + ' <br> ' + d.nama_jenis,
                            d.nama_pabrik,
                            d.nama_operator,
                            d.nama_kondisi,
                            ((d.main_status == 'noschedule' || d.main_status == 'complete' || d.na == 'y') ? '' : 'Sudah terjadwal'),
                            d.nama_operator
                        ]);
                    });
    ...
    function formatRepoSelection(aset) {
        ...
        return aset.text || aset.nomor + ' - ' + aset.nama_kategori + ' - ' + aset.nama_jenis;
    }
    ...
    function formatSearchAset(aset) {
        ...
        var markup = "<div class='select2-result-aset clearfix'>" + aset.nomor + ' <br> ' + aset.nama_kategori + ' <br> ' + aset.nama_jenis + "</div>";
            ...
            ajax: {
                ...
                processResults: function (data) {
                    ...
                    data.data.forEach(function (v) {
                        v.id = v.id_aset;
                        v.text = v.nomor + ' || ' + v.nama_kategori + ' || ' + v.nama_jenis;
                    });
