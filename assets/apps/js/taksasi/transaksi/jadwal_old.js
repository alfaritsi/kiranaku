$(document).ready(function() {
	...
	$(document).on("click", ".edit", function (e) {	
		...
		$.ajax({
			...
			success: function(data){
				console.log(data);
				$(".title-form").html("Form Jadwal BOKIN");
				$.each(data, function(i, v){
					$("#id_jadwal").val(v.id_jadwal);
					$("input[name='nama']").val(v.nama);
					$("select[name='id_tahap']").val(v.id_tahap).trigger("change.select2");
					$("input[name='pra_syarat']").val(v.pra_syarat);
    ...
    $(document).on("click", "button[name='add_grade']", function () {
		let id_tahap = $("select[name='id_tahap']").val();
		if(id_tahap){
    ...
    var mydDatatables = $("#sspTable").DataTable({
        ...
        columns: [
            ...
            },
            {
                ...
                "width": "5%",
                "render": function(data, type, row) {
					var url_detail 	= baseURL + "taksasi/transaksi/nilai/" + row.id_data;
					...
					output += "					<li><a href='javascript:void(0)' class='edit' data-id_data='" + row.id_data + "'><i class='fa fa-pencil-square-o'></i> Edit</a></li>";						
					output += "					<li><a href='"+url_detail+"' ><i class='fa fa-clipboard'></i> Input Nilai</a></li>";					
					output += "				</ul>";
					output += "	        </div>";
                    return output;
                }
