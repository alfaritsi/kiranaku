	...
	$.ajax({
		...
		success: function(data) {
			$.each(data, function(i, v) {
				...
				$("input[name='tanggal_approve_ceo']").val(v.tanggal_approve_ceo);
				$("input[name='approve_ceo']").val(v.approve_ceo);
				...
				if(v.jenis_pengajuan=='pembukaan'){
					...
					if((v.login_buat==id_user)&&(v.status!=99)&&((v.status==1)||(v.status==3))&&(act=='edt')){	
						...
						$('#pendamping_new').prop('required', true);
