        ...
        columns: [
            ...
            },
            {
                ...
                "render": function(data, type, row) {
					if(row.status==99){
						...
						if((row.status_sap==null)&&((row.nomor_rekening==""))){
							if(row.asal_pengajuan=='pabrik'){
								var label_status_rekening = '<span class="label label-warning">Nomor Rekening</span><br><small>Sedang diproses oleh Manager Kantor</small>';
							}else{
								var label_status_rekening = '<span class="label label-warning">Nomor Rekening</span><br><small>Sedang diproses oleh Finance Operation Department Head</small>';
							}
						}else if((row.status_sap==null)&&((row.nomor_rekening!=""))){
							var label_status_rekening = '<span class="label label-danger">Unsync</span>';
						}else{
							var label_status_rekening = '<span class="label label-success">Sync</span>';
						}
						//label status coa
						if(row.no_coa==''){
							var label_status_coa = '<span class="label label-warning">Nomor COA</span><br><small>Sedang diproses oleh Dept Head Accounting</small>';
						}else{
							var label_status_coa = '';
						}
						
						return row.label_status+'<br>'+label_status_rekening+'<br>'+label_status_coa;
					}else{
            ...
            },
            {
                ...
                "render": function(data, type, row) {
					...
					if((row.status==99)&&(row.status_sap==null)&&(row.level==0))
					output += "					<li><a href='javascript:void(0)' class='coa' data-id_data='" + row.id_data_temp + "'><i class='fa fa-list-alt'></i> Set No COA</a></li>";						
					if((row.status==99)&&(row.status_sap==null)&&(row.login_buat==row.id_user))
					output += "					<li><a href='javascript:void(0)' class='rekening' data-id_data='" + row.id_data_temp + "'><i class='fa fa-list-alt'></i> Set Rekening</a></li>";						
