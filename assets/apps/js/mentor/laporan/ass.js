$(document).ready(function() {
	//export to excel
    $(document).on('click', '#export_excel_ass', function (e) {
        e.preventDefault();
        window.open(
            baseURL + 'mentor/laporan/excel/ass/'+'?filter_status='+$('#filter_status').val()
        );
    ...
    var mydDatatables = $("#sspTable").DataTable({
        ...
        columns: [
            ...
            },
            {
                ...
                "render": function(data, type, row) {
					if(row.url_file!=null){
						return "<a href='"+baseURL+""+row.url_file+"' target='_blank'>Dokumen AIM</a>"; 
					}else{
						return "-";
					}
                }
