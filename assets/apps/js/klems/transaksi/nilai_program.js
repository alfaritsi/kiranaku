$(document).ready(function(){
	...
	// $(".set_done").on("click", function(e){
    	...
    	// $.ajax({
    		// url: baseURL+'klems/transaksi/save/set_done',
			// type: 'POST',
			// dataType: 'JSON',
			// data: {
				// status : 'Done',
	...
	$(".set_done").on("click", function(e){
		...
		$.ajax({
			...
			success: function(data){
				console.log(data);
					$("input[name='nama_program']").val(data.nama_program);
					$("input[name='kode']").val(data.kode);
					$("input[name='nama']").val(data.nama);
					$("input[name='id_program_batch']").val(data.id_program_batch);
					$("input[name='list_peserta']").val(data.peserta);
					$("input[name='list_peserta_tambahan']").val(data.peserta_tambahan);
					$("input[name='tahun']").val(data.tahun);
					$("input[name='bulan']").val(data.bulan);
					$("input[name='jenis_sertifikat']").val(data.jenis_sertifikat);
					$('#add_set_done').modal('show');
			}
		});
    });
	$(document).on("click", "button[name='action_btn_set_done']", function(e){
		var empty_form = validate('.form-transaksi-set_done');
		//alert(empty_form);
		if(empty_form == 0){
	    	var isproses 		= $("input[name='isproses']").val();
	    	if(isproses == 0){
	    		$("input[name='isproses']").val(1);
		    	var formData = new FormData($(".form-transaksi-set_done")[0]);
				console.log(formData);
				$.ajax({
					url: baseURL+'klems/transaksi/save/set_done',
					type: 'POST',
