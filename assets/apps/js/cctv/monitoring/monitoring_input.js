...
$(document).ready(function () {
    ...
    /*$('#filtertahun').datepicker({
    	...
    	var pabrik  = $("#filterpabrik").val() != undefined ? $("#filterpabrik").val() : null;
	...
	$(document).on("change", " #filterpabrik", function(e){
        //var tahun       = $("#tahun").val();
 ...
 //    	// initialize(theFilex,idspan);   
			...
			success: function (data) {
				if(data && week!= 0) {
					...
					$.each(data, function(i,v){
						...
						// replace all space
						var idtabpaned = (v.nama).replace(/ /g,"");
						dataheader += '<li class="litab '+activepane+'" id="'+idtabpaned+'"><a href="#'+idtabpaned+'page" '
								...
								success: function (data) {
									...
									if(data) {
										...
										$.each(data, function(i,v){
											...
											if(i < 1){activepane = 'active ';}
											// replace all space
											var idtabpane = arraylok_parent[i].replace(/ /g,"");
											datadetail += '<div class="'+activepane+'tab-pane" id="'+idtabpane+'page">';	
			...
			if (isproses == 0) {
				...
				$.ajax({
					...
					beforeSend: function(){
						// add proces spiner
						var overlay = "<div class='overlay'><i class='fa fa-refresh fa-spin'></i></div>";
				        $("#add_monitoring_modal").append(overlay);
					},
					success: function (data) {
						// console.log(data);
						if (data.sts == 'OK') {
							kiranaAlert(data.sts, data.msg);
							$("#add_monitoring_modal").modal('hide'); 
							// location.reload();
						} else {
					...
					complete: function(){
						$("#add_monitoring_modal .overlay").remove();
					}
	...
	//delete action
	// $(document).on("click", "#delete-group-monitoring", function (e) {
	// 	// var tahun = $('#tahun_sync').val();
	// 	var valuehide = $(this).data("edit");
		
	// 	$.ajax({
	// 		url: baseURL + "cctv/monitoring/set/delete_temp",
	// 		type: 'POST',
	// 		dataType: 'JSON',
	// 		data: {
	// 			kode: valuehide,
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            $("#div_mainTable").append(""
                            ...
                            +'	</tr>'
                            +'	<tr>'
                            +'    <th class="text-align-top text-center" data-headerweek = "w1_m1" width="7%">W1</th>'                              
                            +'    <th class="text-align-top text-center" data-headerweek = "w2_m1" width="7%">W2</th>'
                            +'    <th class="text-align-top text-center" data-headerweek = "w3_m1" width="7%">W3</th>'
                            +'    <th class="text-align-top text-center" data-headerweek = "w4_m1" width="7%">W4</th>'
                            +'    <th class="text-align-top text-center" data-headerweek = "w5_m1" width="7%">W5</th>'
                            +'    <th class="text-align-top text-center" data-headerweek = "w1_m2" width="7%">W1</th>'                              
                            +'    <th class="text-align-top text-center" data-headerweek = "w2_m2" width="7%">W2</th>'
                            +'    <th class="text-align-top text-center" data-headerweek = "w3_m2" width="7%">W3</th>'
                            +'    <th class="text-align-top text-center" data-headerweek = "w4_m2" width="7%">W4</th>'
                            +'    <th class="text-align-top text-center" data-headerweek = "w5_m2" width="7%">W5</th>'    
                            +'	</tr>'                        
                            ...
                            $.each(data.var3,function(i,val) {
                            	if(datas == ''){
                            		first_dot = val.dot;
                            	}
                            	datas_dot.push(val.dot);
                            })*/
                          	$.each(data.var3, function(idot,vdot){
                          		// console.log(datas);
                          		if(datas == 0){
                            		first_dot = vdot.dot;
                            		datas = 1;
                            	}
                            	// console.log(datas,first_dot);
                          		$("#divOut").append("<tr data-tt-id='"+vdot.dot+"'><td>"+vdot.dot+"</td>"
                          							+"<td class='text-center' data-mon='"+vdot.dot+"_w1_m1'></td>"
                          							+"<td class='text-center' data-mon='"+vdot.dot+"_w2_m1'></td>"
                          							+"<td class='text-center' data-mon='"+vdot.dot+"_w3_m1'></td>"
                          							+"<td class='text-center' data-mon='"+vdot.dot+"_w4_m1'></td>"
                          							+"<td class='text-center' data-mon='"+vdot.dot+"_w5_m1'></td>"
                          							+"<td class='text-center' data-mon='"+vdot.dot+"_w1_m2'></td>"
                          							+"<td class='text-center' data-mon='"+vdot.dot+"_w2_m2'></td>"
                          							+"<td class='text-center' data-mon='"+vdot.dot+"_w3_m2'></td>"
                          							+"<td class='text-center' data-mon='"+vdot.dot+"_w4_m2'></td>"
                          							+"<td class='text-center' data-mon='"+vdot.dot+"_w5_m2'></td>"
                          							+"</tr>"); 	                          		
                          	});
                          	...
                          	$.each(data.var1, function(i, v) {
							    ...
							    if (v.year+'-'+month1 == m1 && v.plant == pabrik) {
							    	if(jQuery.inArray(v.week, wk1 ) == -1){
							     		wk1.push(v.week);
							    	}
							    }
							    if (v.year+'-'+month1 == m2 && v.plant == pabrik) {
							    	if(jQuery.inArray(v.week, wk2 ) == -1){
							        	wk2.push(v.week);
							        }
							    }
							});
                          	var maxwk1 = Math.max.apply(null,wk1);
                          	var maxwk2 = Math.max.apply(null,wk2);
                          	week_maks1 = maxwk1;
                          	var week_opt = week_maks1;
                          	if(w == '1'){
                  				jumlah_edit = 2;
                  				week_opt=week_opt-1; 
                  			} else if(w == 2){
                          	...
                          	// console.log(jumlah_edit,maxwk1 , maxwk2, week_maks1, week_opt);
                          	//loop for data
                          	$.each(data.var1, function(i,v){
                          		var month1 = String(v.month);
                          		if(month1.length == 1) month1 = "0"+month1; else month1 = month1;
                          		// set array m-1
								if(v.year+'-'+month1 == m1 && v.plant == pabrik){
									// if(v.dot == first_dot){console.log("pertama")}
									datas_m1.push({
                          				plant 		: v.plant,
                          				dot 		: v.dot,                          				
									...
									if(v.condition == 'ON'){
	                          		...
	                          		// ayy 11.01.2019
	                          		if(ho == 'y'){
	                          			//console.log(jumlah_edit+'|'+first_dot , v.week+"|"+week_maks1 , datas_m2.length+"|"+w, datamon);
	                          			// 0|Penerimaan Bokar 1|1 0|1
	                          			
	                          			// if(v.dot == first_dot){console.log("masuk") ; console.log(w , jumlah_edit , week_opt, v.week);	}
	                          			if(v.dot == first_dot && jumlah_edit > 0 && (v.week == week_opt || v.week == week_maks1) ){//aktifkan ketika live //&& ((datas_m2.length > 1 && w > 1) || ((w==1) && v.month == bulan_sekarang) )
	                          				// console.log(w , jumlah_edit , week_opt, v.week);
	                          				// editaction = "<a href='#' class='edit-group-monitoring' data-edit='"+datamonth_ho1+"'><i class='fa fa-pencil-square-o'></i>Edit </a>";	
	                          				// editaction += "<a href='#' class='delete-group-monitoring' data-delete='"+datamonth_ho1+"'><i class='fa fa-trash'></i>Delete </a>";	
	                          				action = ""
						                        		+"<li><a href='#' class='edit-group-monitoring' data-edit='"+datamonth_ho1+"'><i class='fa fa-pencil-square-o'></i>Edit </a></li>"
						                              	// +"<li><a href='#' class='delete-group-monitoring' data-delete='"+datamonth_ho1+"'><i class='fa fa-trash'></i>Delete </a></li>"
						                            ;

						                    editaction = "<div class='input-group-btn'>"
                          	...
                          	$.each(data.var1, function(i,v){
                          		...
                          		if(v.year+'-'+month2 == m2 && v.plant == pabrik){
									...
									if(v.condition == 'ON'){
	                          		...
	                          		// ayy 11.01.2019
	                          		// console.log('('+v.dot+' == '+first_dot+' &&  ('+Number(v.week)+' == '+Number(w)+' || '+Number(v.week)+' == '+Number(minggulalu)+' )');
	                          		if(ho != 'y') {
		                          		...
		                          		// if(v.week == w){
											...
											// editaction += "<a href='#' class='delete-group-monitoring' data-delete='"+datamonth_ho2+"'><i class='fa fa-trash'></i>Delete </a>";	
											action = ""
						                        		+"<li><a href='#' class='edit-group-monitoring' data-edit='"+datamonths+"'><i class='fa fa-pencil-square-o'></i>Edit </a></li>"
						                              	// +"<li><a href='#' class='delete-group-monitoring' data-delete='"+datamonths+"'><i class='fa fa-trash'></i>Delete </a></li>"
						                            ;
										...
										$('th[data-headerweek="w'+w+'_m2"]').html('W'+w+' <br>'+editaction);
	                          			...
	                          			// console.log(v.dot+'|'+first_dot , v.week+"|"+w , v.week+"|"+minggulalu);
	                          			// console.log(v.week+" == "+minggulalu+" == "+w);
	                          			// if(v.week > batas_edit && v.week <= w && v.dot == first_dot && jumlah_edit < 2){ //aktifkan ketika live
	                          			if(v.dot == first_dot &&  (Number(v.week) == Number(w) || Number(v.week) == Number(minggulalu) || Number(v.week) == Number(minggulalu2) ) ){ //aktifkan ketika live
	                          					
	                          				action = ""
						                        		+"<li><a href='#' class='edit-group-monitoring' data-edit='"+datamonth_ho2+"'><i class='fa fa-pencil-square-o'></i>Edit </a></li>"
						                              	+"<li><a href='#' class='delete-group-monitoring' data-delete='"+datamonth_ho2+"'><i class='fa fa-trash'></i>Delete </a></li>"
						                            ;
