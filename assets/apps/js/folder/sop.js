...
$(document).ready(function() {
    ...
    if (key != "") {
			...
			success: function(data){
				...
				if ((data.parent_admin_akses == 'yes') || (data.parent_admin_akses == 'no' && 
					...
					swal({
						...
						function(){
    						location.href= baseURL+'folder/manage/sop';
				    });
				}
			}
		});
	    ...
	    //CEK AKSES WRITE FOLDER SOP UNTUK MUNCULIN TOMBOL TOOLBAR
	    $.ajax({
			url: baseURL+'folder/manage/cek_akses_root_sop',
			type: 'POST',
			dataType: 'JSON',
			data: {
				id_folder : 131,
	...
	$(document).on("dblclick", ".datatable-folder tr", function(e){
    	...
    	if (id_folder == 0) { // root folder tidak ada perintah write, hanya admin it
    		...
    		$("#icon_new_folder").addClass("hidden");
    		$("#icon_upload").addClass("hidden");
			$("#title-top").html("<strong>STANDARD OPERATING PROCEDURE</strong>");
    	...
    	if(parent == 131 ){
        	...
        	sessionStorage.setItem("folder_admin", 131);
        	sessionStorage.setItem("id_new_folder", id_folder);
        	
    	}else if((parent != 131) && (tipe == 'nav' || tipe == 'Folder')){
        	sessionStorage.setItem("id_new_folder", id_folder);
		...
		if (tipe == 'Folder' || tipe == 'nav') {
	    	...
	    	$.ajax({
				...
				success: function(data){
					...
					$.each(data, function (i, v) {
						if(i == "grandparent" && v.id_folder != "131"){
	...
	*/
	...
	$('#sosialisasi').on("change", function(e){
		var sosialisasi = $(this).val();
		if(sosialisasi == 'ho'){
			$("textarea[name='catatan_kaki']").attr("readonly", true);
			$("textarea[name='catatan_kaki']").val('Sosialisasi akan dilakukan serentak ke seluruh Pabrik oleh Process Owner Head Office. Terimakasih atas perhatian dan kerjasamanya.');
		}else if(sosialisasi == 'pabrik'){
			$("textarea[name='catatan_kaki']").attr("readonly", true);
			$("textarea[name='catatan_kaki']").val('Mohon untuk dapat disosialisasikan ke masing - masing tim pelaksana dengan jangka waktu 14 (empat belas) hari sejak tanggal upload. Terimakasih atas perhatian dan kerjasamanya.');
		}else{
			$("textarea[name='catatan_kaki']").val('');
			$("textarea[name='catatan_kaki']").removeAttr("readonly");
		}
	});
	...
	$("#reupload").on("click", function(){
		$.ajax({
			...
			success: function(data){
				...
				switch (data.sosialisasi) {
					case 'ho' : sosialisasi = "Sosialisasi HO";
						break;
					case 'pabrik': sosialisasi = "Sosialisasi Pabrik";
						break;
					case 'none': sosialisasi = "Tanpa Sosialisasi";
						break;
					default: sosialisasi = "";
						break;
				}

				$("input[name='current_file']").val(data.nama+'.'+data.tipe);
				$("textarea[name='reupload_catatan_kaki']").val(data.catatan_kaki);
				$("input[name='reupload_sosialisasi']").val(sosialisasi);
			}
		});

		$("input[name='id_file']").val(id);
    	$('#reupload_modal').modal('show');		
	});

	$('#fileReupload').on("change", function(e){
		var nama = "";
		$.each($(this).get(0).files, function(i,v){
            nama = v.name;
        });
	...
	$(document).on("click", "button[name='submit_reupload']", function(e){
		var current_file = $("input[name='current_file']").val();
		var new_file     = $("input[name='name']").val();

		if(current_file !== new_file){
			kiranaAlert("notOK", "The name and type must be the same.", "warning", "no");
			e.preventDefault();
			return false;
		}
		
		var empty_form = validate("#form-reupload");
        if(empty_form == 0){

        	var isproses 	= $("input[name='isproses']").val();
			if(isproses == 0){
	    		$("input[name='isproses']").val(1);
				var formData = new FormData($("#form-reupload")[0]);
		    	$.ajax({
		    		url: baseURL+'folder/manage/reupload',
					type: 'POST',
					dataType: 'JSON',
					data:  formData,
	                contentType: false,
					...
					beforeSend: function () {
						var overlay = "<div class='overlay'><i class='fa fa-refresh fa-spin'></i></div>";
						$("button[name='submit_reupload']").addClass("overlay-wrapper").append(overlay);
					},
					success: function(data){
						// console.log(data);
						if(data.sts == 'OK'){
							kiranaAlert(data.sts, data.msg, "success", "no");
							$('#reupload_modal').modal('hide');
							generate_table_after_action();
	    					$("input[name='isproses']").val(0);
						}else{
							kiranaAlert(data.sts, data.msg, "error", "no");
					...
					complete: function () {
						$("button[name='submit_reupload']").removeClass("overlay-wrapper");
						$('.overlay').remove();
					}
				});
    ...
    $(document).on('hide.bs.modal','#reupload_modal', function () {                 
    	$(this).find('#form-reupload')[0].reset();
	});
    ...
    //========================SETTING AKSES KHUSUS ROOT SOP====================================
    $("#set_aksessop").on("click", function(){
		$.ajax({
    		url: baseURL+'folder/manage/get_data_akses_folder',
			type: 'POST',
			dataType: 'JSON',
