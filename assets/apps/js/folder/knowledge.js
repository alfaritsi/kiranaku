...
$(document).ready(function() {
    ...
    //tambahan untuk notifikasi knowledge
    ...
    if (key != "") {
			...
			success: function(data){
				...
				if ((data.parent_admin_akses == 'yes') || (data.parent_admin_akses == 'no' && 
					...
					swal({
						...
						function(){
    						location.href= baseURL+'folder/manage/knowledge';
				    });
				}
			}
		});
	    ...
	    //CEK AKSES WRITE FOLDER KNOWLEDGE UNTUK MUNCULIN TOMBOL TOOLBAR
	    $.ajax({
			url: baseURL+'folder/manage/cek_akses_root_knowledge',
			type: 'POST',
			dataType: 'JSON',
			data: {
				id_folder : 297,
							...
							if(i == "file"){
		                        ...
		                        switch (tipe) {
		                            ...
		                            case 'xlsx': fileClass = "fa-file-excel-o icons_green";
		                                break;
									case 'ppt' :
									case 'pptx': fileClass = "fa-file-powerpoint-o icons_red";
										break;
									case 'mp3' : fileClass = "fa-music icons_blue";
										break;
		                            case 'mp4' :
	...
	$(document).on("dblclick", ".datatable-folder tr", function(e){
    	...
    	if (id_folder == 0) { // root folder tidak ada perintah write, hanya admin it
    		...
    		$("#icon_new_folder").addClass("hidden");
    		$("#icon_upload").addClass("hidden");
			$("#title-top").html("<strong>KNOWLEDGE MANAGEMENT</strong>");
    	...
    	if(parent == 297 ){
        	...
        	sessionStorage.setItem("folder_admin", 297);
        	sessionStorage.setItem("id_new_folder", id_folder);
        	
    	}else if((parent != 297) && (tipe == 'nav' || tipe == 'Folder')){
        	sessionStorage.setItem("id_new_folder", id_folder);
    	...
    	if ($.inArray(tipe, ['mp4', 'webm', 'mp3']) >= 0) {
    		id_folder = $(this).data("parent");
			var id_file = $(this).data("id");
    		$.ajax({
	    		url: baseURL+'folder/manage/get_prev_name',
				...
				success: function (data) {
					...
					output += '			<source src="'+data.address+'" type="'+(tipe == 'mp3' ? 'audio' : 'video')+'/'+tipe+'"></source>';
					output += '		</video>';
		...
		if (tipe == 'Folder' || tipe == 'nav') {
	    	...
	    	$.ajax({
				...
				success: function(data){
					...
					$.each(data, function (i, v) {
						if(i == "grandparent" && v.id_folder != "297"){
	...
	*/
	...
	$("#openFile").on("click", function(){
		if (tipe == 'pdf') {
			...
			$('#view_modal').modal('show');		
		} else if ($.inArray(tipe, ['mp4', 'webm', 'mp3']) >= 0) {
			$.ajax({
	    		url: baseURL+'folder/manage/get_prev_name',
				type: 'POST',
				dataType: 'JSON',
				...
				success: function (data) {
					...
					output += '	<div class="tab-pane active"id="video-tab">';
					output += '		<video class="video-js" controls controlsList="nodownload" style="width: 100%; height: 400px" preload="metadata">';
					output += '			<source src="'+data.address+'" type="video/'+tipe+'"></source>';
					output += '		</video>';
					output += '		<code>Total Views: '+(data.log.length > 0 ? data.log[0].total_count : '0')+'</code>';
					output += '	</div>';
					output += '	<div class="tab-pane"id="detail-tab">';
    ...
    //========================SETTING AKSES KHUSUS ROOT KNOWLEDGE====================================
    $("#set_aksesknowledge").on("click", function(){
		$.ajax({
    		url: baseURL+'folder/manage/get_data_akses_folder',
			type: 'POST',
			dataType: 'JSON',
