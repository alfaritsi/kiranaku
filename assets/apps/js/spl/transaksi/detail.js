$(document).ready(function () {
    ...
    $(document).on("click", "button[name='action_btn']", function (e) {
        ...
        const message = (action == "reject") ? "Tolak" : "Setujui";
        kiranaConfirm({
            title: "SPL",
            text: message + " Pengajuan SPL?",
            dangerMode: true,
            useButton: null,
            ...
            successCallback: function () {
                if (action == "approve") {
                    $("textarea[name='komentar']").prop("required", false);
                    $("input[name='status_detail[]']").prop("required", true);
                    const jumlahKaryawan = $("input[name='nik[]']").length;
                    if (jumlahKaryawan < 1) {
                    ...
                    $("textarea[name='komentar']").prop("required", true);
                    $("input[name='status_detail[]']").prop("required", false);
                    saveSpl();
...
const getData = () => {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data) {
                if (data.access != 1 || data.realisasi == 1)
                ...
                if (data.detail) {
                    ...
                    $.each(data.detail, function (i, v) {
                        ...
                        output += ' </td>';
                        output += ' <td><input type="time" class="form-control timepicker input-edit" name="jam_mulai[]" value="' + v.jam_mulai_format + '" readonly required></td>';
                        output += ' <td><input type="time" class="form-control timepicker input-edit" name="jam_selesai[]" value="' + v.jam_selesai_format + '" readonly required><span class="help-block" style="color: red;"></span></td>';
                        ...
                        if (data.access == 1 && data.status_spl !== "finish") {
                            output += ' <td>';
                    ...
                    if (data.detail_realisasi && data.detail_realisasi.length > 0) {
                        $("#jumlah_orang").val(data.detail_realisasi.length);
                        $.each(data.detail_realisasi, function (i, v) {
                            // $("#nodata").remove();
                            let output = "";
                            output += '<tr class="row-karyawan-realisasi">';
                            output += ' <td><input type="text" class="form-control text-center" readonly value="' + (i + 1) + '"></td>';
                            output += ' <td>';
                            output += '     <input type="text" class="form-control" value="' + v.nik + ' - ' + v.nama + '" required readonly>';
                            output += ' </td>';
                            output += ' <td><input type="text" class="form-control" value="' + v.posst + '" readonly></td>';
                            output += ' <td><input type="text" class="form-control text-center" value="' + v.sebelum_spl_jam + ":" + ('00' + v.sebelum_spl_menit).slice(-2) + '" readonly></td>';
                            output += ' <td><input type="time" class="form-control" value="' + v.jam_mulai_format + '" readonly required></td>';
                            output += ' <td><input type="time" class="form-control" value="' + v.jam_selesai_format + '" readonly required></td>';
                            output += ' <td><input type="text" class="form-control text-center" value="' + v.sesudah_spl_jam + ":" + ('00' + v.sesudah_spl_menit).slice(-2) + '" readonly></td>';
                            output += '</tr>';
                            $(output).appendTo("#list-karyawan-realisasi");
                        });
                    }
                }
...
const saveSpl = () => {
    ...
    if (empty_form == 0) {
        ...
        if (isproses == 0) {
            ...
            $.ajax({
                url: baseURL + 'spl/transaksi/save/approval',
                type: 'POST',
                dataType: 'JSON',
                data: formData,
                contentType: false,
