...
$(document).ready(function () {
    ...
    $('#table-cancel').DataTable({
        ...
        'order': [[1, 'asc']],
        "oSearch": {"sSearch": $('#table-cancel').attr('data-search')}
    });

    $(document).on("click", "#btn-sync-declaration", function (e) {
        // console.log('masuk');
        e.preventDefault();
        ...
        if (isproses == 0) {
            ...
            let chosenCatatans = [];
            $.each($('#table-declare').DataTable().column(0).checkboxes.selected(), function (i, value) {
                chosenCatatans.push({
                    id: value,
            ...
            if(chosenCatatans == "") {
                ...
                $.ajax({
                    url: baseURL + 'travel/sync/save/approve_declare',
                    type: 'POST',
                    dataType: 'JSON',
                    data: {
