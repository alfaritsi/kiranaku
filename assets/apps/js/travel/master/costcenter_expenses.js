$(document).ready(function () {
    ...
    $(document).on("click", ".edit", function (e) {
        ...
        const form = $('.form-costcenter-expenses');
        KIRANAKU.showLoading();
        $.ajax({
            url: baseURL + 'travel/spd/get/costcenter_expenses',
            type: 'POST',
            dataType: 'JSON',
            data: {
                id: id
            ...
            success: function (data) {
                if (data.sts === 'OK') {
                    ...
                    $('input[name="id_travel_costcenter_expense"]', form).val(editData.id_travel_costcenter_expense);
                    $('#personal_area', form)
        ...
        kiranaConfirm(
            {
                ...
                successCallback: function () {
                    KIRANAKU.hideLoading();
                    $.ajax({
                        url: baseURL + 'travel/spd/delete/costcenter_expenses',
                        type: 'POST',
                        dataType: 'JSON',
                        data: {
                            id: id
    ...
    $(document).on("click", "button[name='action_btn']", function (e) {
        const form = $('.form-costcenter-expenses');
        validate('.form-costcenter-expenses', true);
        ...
        if (valid) {
            ...
            if (isproses == 0) {
                ...
                $.ajax({
                    url: baseURL + 'travel/spd/save/costcenter_expenses',
                    type: 'POST',
                    dataType: 'JSON',
                    data: formData,
                    contentType: false,
    ...
    $(document).on('click', '.select2-all', function (e) {
        const inpGroup = $(this).parents('.input-group');
