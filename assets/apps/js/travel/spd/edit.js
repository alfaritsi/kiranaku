    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.sts === 'OK') {
                ...
                //START FORM HEADER
                //tab um
                let totalum = pengajuan.total_um > 0 ? parseInt(pengajuan.total_um) : '0';
                // $('input[name="total_um"]').val(rupiah(totalum));
                AutoNumeric.set('input[name="total_um"]', totalum);
                //AKTIFITAS
                ...
                if (tipe_trip == 'single') {
                    ...
                    $('input[name="detail[' + rowtrip + '][id]"]').val(details.id_travel_detail);

                    if (detail.tujuan_lain !== "" && detail.tujuan_lain.trim() !== "") {
                        $('input[name="detail[' + rowtrip + '][tujuan_lain]"]').closest('.form-group').removeClass('hide');
                    }

                    let endDatetime = moment(detail.end_date + " " + detail.end_time).format("DD.MM.YYYY HH:mm");
                    if ($("#div_kembali").hasClass('hide')) {
                        $("#div_kembali").removeClass('hide');
                    }

                    let startDatetime = moment(detail.start_date + " " + detail.start_time).format("DD.MM.YYYY HH:mm");

                    $('input[name="detail[' + rowtrip + '][start]"]').parents('.trip_start_datetime_multi').data("DateTimePicker").date(startDatetime);
                    $('.trip_end_datetime_multi').data("DateTimePicker").date(endDatetime);
                ...
                if (downpayments.length) {
                    ...
                    $.each(downpayments, function (i, dp) {
                        ...
                        const expense = $(template);

                        const rateV = parseFloat(dp.value) * 100;

                        let el = $('.uangmuka-label-jumlah', expense);
                        $('.uangmuka-id', expense).val(dp.id);
                        $('.uangmuka-kode_expense', expense).val(dp.kode_expense);
                        $('.uangmuka-durasi', expense).val(dp.durasi);
                        $('.uangmuka-rate', expense).val(rateV);
                        ...
                        // AutoNumeric.set(el, parseFloat(dp.jumlah));
                        $('.uangmuka-jumlah', expense).val(dp.jumlah);
                        const jumlah = parseFloat(KIRANAKU.isNullOrEmpty(dp.jumlah, dp.jumlah, 0));
                        totalUangmuka += jumlah;
                        $('.uangmuka-jumlah', expense).val(jumlah);
                        totalMaxUangmuka += (durasiSpd * rateV);
                        if (rateV != 0) {
                            $('.uangmuka-jumlah', expense).attr('numeric-max', durasiSpd * rateV);
                        }
                        $('.uangmuka-currency', expense).val(dp.currency);
                ...
                if (rencana_aktifitas && rencana_aktifitas.length) {
                    ...
                    $.each(rencana_aktifitas, function (i, v) {
                        let output = `<tr class="row-aktifitas">
                            <td><input type="text" class="form-control" name="tanggal_aktifitas_add[]" autocomplete="off" required readonly value="${v.tanggal_aktifitas_format}"></td>
                            <td><input type="text" class="form-control" name="pabrik_aktifitas_add[]" autocomplete="off" required value="${v.lokasi}"></td>
                            <td><textarea type="text" class="form-control" name="aktifitas_add[]" rows="1" required>${v.aktifitas}</textarea></td>
                        </tr>`;
...
function generate_data_trip_edit(data, rowtrip, single, detail) {
    ...
    output += '		            <div class="input-group">';
    output += '		                <span class="input-group-addon"><span class="fa fa-pencil colors-peach"></span></span>';
    output += '		                <textarea style="height:37px !important;" maxlength="55" class="form-control input-keperluan readonly" name="detail[' + rowtrip + '][keperluan]" id="detail[' + rowtrip + '][keperluan]" required>' + detail.keperluan + '</textarea>';
    output += '		            </div>';
    output += '		        </div>';
    output += '		    </div>';
