...
$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.sts === 'OK') {
                const { pengajuan, details, optpabrik, opttujuan, optcountry, cancel, deklarasi, history, downpayments, expenses, expenses_currency, personel } = data.data;
                ...
                var start_date_label = moment(pengajuan.start_date).format("DD.MM.YYYY");
                $('input[name="start_date_label"]').val(start_date_label);
                var start_date_label_hidden = moment(pengajuan.start_date);
                $('input[name="start_date_label_hidden"]').val(start_date_label_hidden);
                var end_date_label = moment(pengajuan.end_date).format("DD.MM.YYYY");
                $('input[name="end_date_label"]').val(end_date_label);
                var toDate = new Date(pengajuan.end_date);
                var fromDate = new Date(pengajuan.start_date);
                var durasi_spd = dateDiff('d', fromDate, toDate) + 1;
                ...
                if (tipe_trip == 'single') {
                    ...
                    generate_data_trip_disabled(data, rowtrip, tipe_trip, detail);
                ...
                $('.biaya_tanggal input', newBiaya)
                    .val(
                        moment(pengajuan.tanggal).format('DD.MM.YYYY')
                    );
                ...
                $.each(expensesOptions, function (i, val) {
                    if (val.id == 'DUSK') {
                        ...
                        option.setAttribute('data-biaya', JSON.stringify(val.data));
                        $('.select-biaya', newBiaya).append(option);
                        // $('.biaya-rate', newBiaya).val(rupiah(val.nilai));
                        $("input[name='biaya[0][rate]']").val(rupiah(val.nilai));
                        $('.biaya-jumlah', newBiaya).val(parseFloat(val.nilai * durasi_spd));

                        $("input[name='biaya[0][lampiran]']").prop('required', false);
                        $("button[name='biaya[0][btn_delete]']").addClass('disabled');

                        if (val.nilai != 0) {
                            $('.biaya-jumlah', newBiaya).attr('numeric-max', val.nilai * durasi_spd);
                        }
    ...
    $(document).on("click", "#sbmit_xx", function (e) {
        var isproses = $("input[name='isproses']").val();
        if (isproses == 0) {
            $("input[name='isproses']").val(1);
            KIRANAKU.showLoading();
            var formData = new FormData($("#form-pengajuan-deklarasi")[0]);
            $.ajax({
    ...
    $(document).on("click", "#sbmit", function (e) {
        var empty_form = validate("#form-pengajuan-deklarasi", true);
        if (empty_form == 0) {
            var isproses = $("input[name='isproses']").val();
            if (isproses == 0) {
                $("input[name='isproses']").val(1);
    ...
    KIRANAKU.createValidator($('.form-pengajuan-deklarasi'));

    $(document).on('click', 'button[name="simpan_btn"]', function (e) {
        const modal = $('#modal-spd-pengajuan');
        var form = $('.form-pengajuan-deklarasi');
    ...
    $(document).on('click', 'button[name="simpan_btn_tambah_um"]', function (e) {
        ...
        var form = $('.form-pengajuan-deklarasi-um-tambah');
        const multiTripTotal = multiTripTable.rows().count();
        form.validate();
        var valid = form.valid();
        ...
        if (valid) {
            ...
            if (isproses == 0) {
                KIRANAKU.startProses();
                var formData = new FormData($(".form-pengajuan-deklarasi-um-tambah")[0]);
                KIRANAKU.showLoading();
                $.ajax({
    ...
    $(document).on('click', '.spd-edit', function (e) {
        ...
        const form = $('.form-pengajuan-deklarasi', modal);
        ...
        if (nomor > 0) {
            $.ajax({
                ...
                success: function (data) {
                    if (data.sts === 'OK') {
                        $.each(data.data, function (i, v) {
                            $("input[name='biaya[" + nomor + "][rate]']").val(rupiah(v.value * 100));
                        });
                    }
                }
            });
