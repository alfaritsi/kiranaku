...
$(document).ready(function () {
    ...
    $(document).on("click", "button[name='action_btn']", function (e) {
        ...
        var output = '';
        output += '<form class="form-persetujuan-deklarasi" enctype="multipart/form-data">';
    ...
    $(document).on("click", ".btn-approval", function (e) {
        ...
        if (isproses == 0) {
            const formData = new FormData($(".form-persetujuan-deklarasi")[0]);
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.sts === 'OK') {
                ...
                $("input[name='durasi_label']").val(dateDiff('d', fromDate, toDate) + 1);
                ...
                if (tipe_trip_deklarasi == 'single') {
                    ...
                    generate_data_trip_deklarasi(detail);
                ...
                /** Detail Deklarasi */
                else if (KIRANAKU.isNotNullOrEmpty(pengajuan_deklarasi)) {
                ...
                if (cuti_pengganti && cuti_pengganti.length) {
                    const is_approval_hr = $("input[name='is_approval_hr']").val();
                    $.each(cuti_pengganti, function (i, v) {
                        let del = (is_approval_hr == "1") ? '<button type="button" class="btn btn-sm btn-danger delete_cuti" title="Remove" data-id="' + v.id + '"><i class="fa fa-trash"></i></button>' : '';
                        let output = `<tr class="row-cuti">
                            <td>
                                <div class="input-group ">
                                    <div class="input-group-addon"><i class="fa fa-calendar colors-tosca"></i></div>
                                    <input type="text" data-date-format="dd.mm.yyyy" class="form-control date-cuti" name="tanggal_cuti_add[]" required value="${v.tanggal_cuti_format}" readonly>
                                </div>
                            </td>
                            <td><input type="text" class="form-control" name="cuti_add[]" autocomplete="off" required value="${v.keterangan}" readonly></td>
                            <td>${del}</td>
                        </tr>`;
    ...
    $(document).on("click", ".delete_cuti", function (e) {
        let elem = $(this);
        let id = $(this).attr("data-id");
        kiranaConfirm(
            {
                title: "Konfirmasi",
                text: "Apakah anda akan menghapus data pengajuan cuti?",
                dangerMode: true,
                successCallback: function () {
                    KIRANAKU.hideLoading();
                    $.ajax({
                        url: baseURL + 'travel/spd/delete/cuti_pengganti',
                        type: 'POST',
                        dataType: 'JSON',
                        data: {
                            id: id
                        ...
                        success: function (data) {
                            ...
                            if (data.sts === 'OK') {
                                ...
                                elem.closest("tr.row-cuti").remove();
                                cekListCuti();
                            } else {
                                kiranaAlert(data.sts, data.msg, 'error', 'no');
                            }
...
function generate_data_trip_deklarasi(detail) {
