		...
		$.ajax({
			...
			success: function(data){
				...
				$.each(data, function(i, v){
					...
					$("input[name='nama_keluarga']").val(v.nama_keluarga);
					$("input[name='status_keluarga']").val(v.status_keluarga);
	...
	$(document).on("click", "button[name='action_btn']", function(e){
		...
		if(empty_form == 0){
	    	...
	    	if(isproses == 0){
		    	...
		    	var formData = new FormData($(".form-master-berita")[0]);

				$.ajax({
					url: baseURL+'berita/master/save/duka',
					type: 'POST',
					dataType: 'JSON',
					data: formData,
					contentType: false,
