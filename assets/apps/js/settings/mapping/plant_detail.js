$(document).ready(function(){
	...
	$(".edit").on("click", function(e){
		...
		$.ajax({
    		url: baseURL+'settings/mapping/get/plant_detail',
			type: 'POST',
			dataType: 'JSON',
			data: {
				apps : apps
			...
			success: function(data){
				console.log(data);
				$(".title-form").html("Edit Mapping Plant Detail");
				$.each(data, function(i, v){
					$("input[name='apps']").val(v.apps);
					$("select[name='plant']").val(v.plant).trigger("change.select2");
					if(v.plant_access!==null){
						var plant	= v.plant_access.split(",");
						$("select[name='plant_access[]']").val(plant).trigger("change");
					}
	...
	$(document).on("click", "button[name='action_btn']", function(e){
		...
		if(empty_form == 0){
	    	...
	    	if(isproses == 0){
				...
				$.ajax({
					url: baseURL+'settings/mapping/save/plant_detail',
					type: 'POST',
					dataType: 'JSON',
					data: formData,
					contentType: false,
