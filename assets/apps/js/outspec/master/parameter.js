$(document).ready(function () {
    ...
    $(document).on("click", "button[name='action_btn']", function (e) {
        ...
        if (empty_form == 0) {
            ...
            if (isproses == 0) {
                ...
                $.ajax({
                    url: baseURL + 'outspec/master/save/parameter',
                    type: 'POST',
                    dataType: 'JSON',
                    data: formData,
                    contentType: false,
    ...
    $(document).on('click', '.edit', function (e) {
        ...
        $.ajax({
            url: baseURL + 'outspec/master/get/parameter',
            type: 'POST',
            dataType: 'JSON',
            data: {
                return: 'json',
        ...
        kiranaConfirm(
            {
                ...
                successCallback: function () {
                    $.ajax({
                        url: baseURL + 'outspec/master/set/parameter',
                        type: 'POST',
                        dataType: 'JSON',
                        data: {
                            id: id,
    ...
    $(document).on("click", ".activate", function (e) {
        const id = $(this).attr("data-active");
    ...
    $("#sspTable").dataTable({
        ...
        },
        {
            ...
            render: function(data, type, row) {
                ...
                output += "                 <li><a href='#' class='edit' data-edit='" + row.id + "'><i class='fa fa-pencil-square-o'></i> Edit</a></li>";
                if (row.na == 'n') {
                    output += "                 <li><a href='#' class='activate' data-active='" + row.id + "' data-action='deactivate'><i class='fa fa-minus text-danger'></i> Non Active</a></li>";
                    output += "                 <li><a href='#' class='delete' data-delete='" + row.id + "'><i class='fa fa-trash-o'></i> Hapus</a></li>";
                }else{
                    output += "                 <li><a href='#' class='activate' data-active='" + row.id + "' data-action='activate'><i class='fa fa-check text-success'></i> Set Active</a></li>";
                }
                output += "				</ul>";
                output += "	        </div>";
                return output;
